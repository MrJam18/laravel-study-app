    ...
    <div class="create-news__content">
        ...
        <form method="post" action='{{\route('api/createNews')}}' class="form-group create-news__form">
            <div >
                ...
                <select required id="createNews__category" name="category_id" class="form-control create-news__input" >
                    @foreach ($categories as $category)
                        <option value="{{$category->id}}">{{$category->title}}</option>
                    @endforeach
                </select>
                <label for="createNews__category" class="create-news__label">Источник</label>
                <select required id="createNews__category" name="news_source_id" class="form-control create-news__input" >
                    @foreach ($newsSources as $source)
                        <option  value="{{$source->id}}">{{$source->name}}</option>
                    @endforeach
                </select>
                <label class="create-news__label" for="createNews__header">Заголовок</label>
                <input required type="text" class="form-control create-news__input" name="title" id="createNews__header">
