    ...
    <div class="admin-setter__content">
        ...
        <form method="post" action='{{$vars->actionRoute}}' class="form-group admin-setter__form">
            <div>
                ...
                <input required type="text" value="{{$vars->model->name}}" class="form-control admin-setter__input" name="name" id="name">
                <label for="phone" class="admin-setter__label">Номер Телефона</label>
                <input required type="tel" value="{{$vars->model->phone}}" class="form-control admin-setter__input" name="phone" id="phone">
                <label class="admin-setter__label" for="email">Электронная почта</label>
                <input required type="email" value="{{$vars->model->email}}" class="form-control admin-setter__input" name="email" id="email">
                <label class="admin-setter__label" for="description">Краткое описание запроса</label>
                <textarea class="form-control admin-setter__input" id="description" required rows="2" name="description">{{$vars->model->description}}</textarea>
                <label class="admin-setter__label" for="text">Текст запроса</label>
                <textarea class="form-control admin-setter__input" id="text" required rows="8" name="text">{{$vars->model->text}}</textarea>
